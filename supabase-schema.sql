...
-- 12. Niches Table (For YouTube Channel Tracking)
...
drop policy if exists "Auth Insert Niches" on niches;
drop policy if exists "Auth Delete Niches" on niches;

create policy "Public Read Niches" on niches for select using (true);
-- Allow authenticated users (admins/students) to add niches for now, or restrict to admin if preferred.
-- Based on Dashboard.tsx, it seems adding is open or admin only? 
-- The UI shows add button for everyone but delete for admin only.
-- Let's stick to authenticated users for insert.
create policy "Auth Insert Niches" on niches for insert with check (auth.role() = 'authenticated');
create policy "Auth Delete Niches" on niches for delete using (auth.role() = 'authenticated');
